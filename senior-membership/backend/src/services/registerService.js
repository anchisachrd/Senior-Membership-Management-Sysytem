...
export const register = async (candidateData, heirData) => {
  ...
  try {
    ...
    const candidateAccountID = await accountModel.createAccount({
      ...
      password_hash: candidateData.account.password, // make sure to hash in the model if needed
      role: "candidate",
    });
    ...
    //-------------------------------------------
    // If the user checked "same address," we reuse the Candidate's AddressID.
    // Otherwise, we create a new address record for the Heir.
    ...
    //-------------------------------------------
    // 6) CREATE HEIR ACCOUNT (IF APPLICABLE)
    //-------------------------------------------
    // If you automatically generate a password for the Heir, handle that here.
    // If not, just pass heirData.account.password if user provided it.
    ...
    if (heirData.account && heirData.account.email) {
      heirAccountID = await accountModel.createAccount({
        ...
        password_hash: heirData.account.password || "AUTO_GENERATED_PASSWORD",
        role: "heir",
      });
    }
